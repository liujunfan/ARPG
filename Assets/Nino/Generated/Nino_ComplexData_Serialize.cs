    ...
    public partial class ComplexData
    {
        ...
        public class SerializationHelper: Nino.Serialization.NinoWrapperBase<ComplexData>
        {
                ...
                if(value.f != null)
                {
                    ...
                    foreach (var entry in value.f)
                    {
                        if(entry != null)
                        {
                        	...
                        	foreach (var entry0 in entry)
                        	{
                        		Nino.Data.NinoSerializationHelper.Serialize(entry0, writer);
                        	}
                        }
                        else
                        {
                        ...
                        if(entry != null)
                        {
                        	...
                        	foreach (var entry0 in entry)
                        	{
                        	    if(entry0 != null)
                        	    {
                        		    ...
                        		    {
                        			    Nino.Data.NinoSerializationHelper.Serialize(entry1, writer);
                        		    }
                        	    }
                        	    else
                        	    {
            ...
            public override ComplexData Deserialize(Nino.Serialization.Reader reader)
            {
                ...
                value.f = new Nino.Data[reader.ReadLength()][];
                for(int i = 0, cnt = value.f.Length; i < cnt; i++)
                {
                    var value_f_i = new Nino.Data[reader.ReadLength()];
                	for(int value_f_i_i = 0, value_f_i_cnt = value_f_i.Length; value_f_i_i < value_f_i_cnt; value_f_i_i++)
                	{
                		var value_value_f_i_i = Nino.Data.NinoSerializationHelper.Deserialize(reader);
                		value_f_i[value_f_i_i] = value_value_f_i_i;
                	}
                    value.f[i] = value_f_i;
                }
                value.g = new System.Collections.Generic.List<Nino.Data[]>(reader.ReadLength());
                for(int i = 0, cnt = value.g.Capacity; i < cnt; i++)
                {
                    var value_g_i = new Nino.Data[reader.ReadLength()];
                	for(int value_g_i_i = 0, value_g_i_cnt = value_g_i.Length; value_g_i_i < value_g_i_cnt; value_g_i_i++)
                	{
                		var value_value_g_i_i = Nino.Data.NinoSerializationHelper.Deserialize(reader);
                		value_g_i[value_g_i_i] = value_value_g_i_i;
                	}
                    value.g.Add(value_g_i);
                }
                value.h = new Nino.Data[reader.ReadLength()][][];
                for(int i = 0, cnt = value.h.Length; i < cnt; i++)
                {
                    var value_h_i = new Nino.Data[reader.ReadLength()][];
                	for(int value_h_i_i = 0, value_h_i_cnt = value_h_i.Length; value_h_i_i < value_h_i_cnt; value_h_i_i++)
                	{
                		var value_value_h_i_i = new Nino.Data[reader.ReadLength()];
                		...
                		{
                			var value_value_value_h_i_i_i = Nino.Data.NinoSerializationHelper.Deserialize(reader);
                ...
                value.i = new System.Collections.Generic.List<Nino.Data>[reader.ReadLength()];
                for(int i = 0, cnt = value.i.Length; i < cnt; i++)
                {
                    var value_i_i = new System.Collections.Generic.List<Nino.Data>(reader.ReadLength());
                	for(int value_i_i_i = 0, value_i_i_cnt = value_i_i.Capacity; value_i_i_i < value_i_i_cnt; value_i_i_i++)
                	{
                		var value_value_i_i_i = Nino.Data.NinoSerializationHelper.Deserialize(reader);
                		value_i_i.Add(value_value_i_i_i);
                	}
                    value.i[i] = value_i_i;
                }
                value.j = new System.Collections.Generic.List<Nino.Data[]>[reader.ReadLength()];
                for(int i = 0, cnt = value.j.Length; i < cnt; i++)
                {
                    var value_j_i = new System.Collections.Generic.List<Nino.Data[]>(reader.ReadLength());
                	for(int value_j_i_i = 0, value_j_i_cnt = value_j_i.Capacity; value_j_i_i < value_j_i_cnt; value_j_i_i++)
                	{
                		var value_value_j_i_i = new Nino.Data[reader.ReadLength()];
                		...
                		{
                			var value_value_value_j_i_i_i = Nino.Data.NinoSerializationHelper.Deserialize(reader);
