                ...
                if (path && path.floatingWindowType == GetType())
                {
                    ...
                    using (GUIContentColorScope.New(EditorGUIUtilities.labelNormalColor))
                    {
                        ...
                        if (GUI.Toggle(rect, selectedTool == 0,
                            EditorGUIUtilities.TempContent(null, EditorPathResources.instance.moveToolPan, "Pan Move Tool"),
                            EditorGUIUtilities.buttonLeftStyle))
                            selectedTool = 0;

                        rect.x = rect.xMax;
                        if (GUI.Toggle(rect, selectedTool == 1,
                            EditorGUIUtilities.TempContent(null, EditorPathResources.instance.moveTool3D, "3D Move Tool"),
                        ...
                        using (DisabledScope.New(disableRotateTool))
                        {
                            ...
                            if (GUI.Toggle(rect, selectedTool == 2,
                                EditorGUIUtilities.TempContent(null, EditorPathResources.instance.RotateTool, "Rotate Tool"),
                        ...
                        rect.width /= 3;
                        if (GUI.Button(rect,
                            EditorGUIUtilities.TempContent(null, EditorPathResources.instance.addNodeBack, "Add Node Back"),
                            EditorGUIUtilities.buttonLeftStyle))
                        {
                        ...
                        using (DisabledScope.New(path.nodeCount == 2))
                        {
                            rect.x = rect.xMax;
                            if (GUI.Button(rect,
                                EditorGUIUtilities.TempContent(null, EditorPathResources.instance.removeNode, "Remove Node"),
                                EditorGUIUtilities.buttonMiddleStyle))
                            {
                        ...
                        rect.x = rect.xMax;
                        if (GUI.Button(rect,
                            EditorGUIUtilities.TempContent(null, EditorPathResources.instance.addNodeForward, "Add Node Forward"),
                            EditorGUIUtilities.buttonRightStyle))
                        {
